...
--CTE
WITH vacc_per_populations (location, date, total_cases, population, total_cases_percentage, 
new_vacc, people_vaccinated) AS
(
SELECT death.location, death.date, total_cases, population, (total_cases/population)*100 as total_cases_percentage,
  CAST (new_vaccinations as float) as new_vacc, 
  SUM(CAST(new_vaccinations as float)) OVER (PARTITION BY death.location ORDER BY death.date) as people_vaccinated
  --(CASt(total_vaccinations as float)/population)*100 as vacc_percentage
FROM Portfolio_COVID19..CovidDeath as death
JOIN Portfolio_COVID19..CovidVaccinations as vacc ON 
death.location = vacc.location AND death.date = vacc.date
--ORDER BY 1,2
